...
def student_required(function=None, redirect_field_name=REDIRECT_FIELD_NAME, login_url=None):
    actual_decorator = user_passes_test(
        lambda u: u.is_authenticated and u.user_type.name == "student",
        login_url=login_url,
...
def student_message(function):
    def message_function(request, uuid=None, *args, **kwargs):
        if isinstance(request, WSGIRequest):
            if request.user.is_authenticated and not request.user.user_type.name == "student":
                messages.error(request, "You must login as student to access this page.")
